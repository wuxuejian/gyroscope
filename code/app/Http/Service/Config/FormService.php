...
class FormService extends BaseService
{
        ...
        foreach ($data as $item) {
            ...
            foreach ($item['data'] as $form) {
                ...
                $tmpForm = [
                    ...
                    'value'         => $form['value'] ? (is_array($form['value']) ? json_encode($form['value'], JSON_UNESCAPED_UNICODE) : $form['value']) : $form['value'],
                    'param'         => $form['param'] ?? '',
     ...
     * @throws \ReflectionException
     */
    public function getFormDataWithType(int $types, bool $withOptions = true, string $platform = UserAgentEnum::ADMIN_AGENT, int $associationId = 0): array
    {
        ...
        foreach ($formList as $index => $item) {
            foreach ($item['data'] as $key => $datum) {
                ...
                if ($datum['dict_ident']) {
                    $level = $dictTypeService->value(['ident' => $datum['dict_ident']], 'level');
                    if ($level == 1 && $datum['type'] == 'single') {
                        ...
                        $formList[$index]['data'][$key]['value'] = $datum['value'] ? (is_array(end($datum['value'])) ? call_user_func_array('array_merge', $datum['value']) : $datum['value']) : [];
                    }
                    if ($withOptions) {
                ...
                if ($types == CustomEnum::CONTRACT && $datum['key'] == 'contract_customer') {
                    $level   = 1;
                    $options = app()->get(CustomerService::class)->setPlatform($platform)->getCurrentSelect($associationId);
                }
