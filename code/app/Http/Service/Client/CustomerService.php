...
class CustomerService extends BaseService implements ClientInterface
{
    ...
    public function saveCustomer(array $data, int $customType, int $force): mixed
    {
        ...
        $formService->fieldValueCheck($data, CustomEnum::CUSTOMER, 0, $list, $force);
    ...
    public function updateCustomer(array $data, int $id, int $force): mixed
    {
        ...
        // TODO:待优化
        $list = $formService->getFormDataList(CustomEnum::CUSTOMER);
        $formService->fieldValueCheck($data, CustomEnum::CUSTOMER, $id, $list, $force);
        foreach ($list as $item) {
            $type      = strtolower($item['type']);
            $inputType = strtolower($item['input_type']);
            foreach ($data as $key => &$datum) {
     ...
     * @throws NotFoundExceptionInterface
     */
    public function getListStatistics(int $customType, string $uid, $userIds): array
    {
     ...
     * @throws \ReflectionException
     */
    public function getCurrentSelect(int $id): array
    {
        $nameField = $this->getPlatform() == UserAgentEnum::ADMIN_AGENT ? 'label' : 'text';

        $field = ['id as value', 'customer_name as ' . $nameField];
        $list  = $this->dao->getList(['uid' => uuid_to_uid($this->uuId(false))], $field, 0, 0, 'id', callable: function ($list) use (&$id) {
            foreach ($list as $item) {
                $item->disabled = false;
                if ($item->value == $id) {
                    $id = 0;
        ...
        if ($id) {
            ...
            $info && $list[] = ['value' => $info['id'], $nameField => $info['customer_name'], 'disabled' => $info['uid'] < 1];
        }
