...
class BillService extends BaseService implements ResourceServicesInterface
{
    ...
    public function getList(array $where, array $field = ['*'], $sort = 'id', array $with = []): array
    {
        ...
        if (! $where['cate_id']) {
            ...
            if (is_array($where['cate_id'])) {
                $cateIds = [];
                foreach ($where['cate_id'] as $item) {
                    $item    = json_decode($item, true);
                    $cateIds = array_merge($cateIds, $item);
                }
                $where['cate_id'] = array_unique(array_filter($cateIds));
            }
        }
        ...
        $list        = $this->dao->getList($where, $field, $page, $limit, $sort, [
            ...
            'clientBill' => function ($query) {
                $query->select(['id', 'bill_no', 'cid', 'eid']);
            },
    ...
    {
        return $this->elForm('添加', $this->getRankRuleForm(collect()), '/ent/bill');
    }
    ...
    public function resourceEdit(int $id, array $other = []): array
    {
        ...
        return $this->elForm('编辑', $this->getRankRuleForm(collect($rankInfo->toArray())), '/ent/bill/' . $id, 'put');
    }
    ...
    public function resourceUpdate($id, array $data)
    {
        ...
        if ($res) {
            Task::deliver(new BillListRecordTask($data['entid'], (int) $id, auth('admin')->id(), 1, $info->toArray()));
        }
        return $res;
    }
     ...
     * @throws BindingResolutionException
     */
    public function getTrend($time, $income, $expend, $all = false, $cateId = [])
    {
        $time = explode('-', $time);
        if (count($time) != 2) {
        ...
        $data     = [];
        if ($dayCount == 1) {
            $data = $this->trend($time, 0, $income, $expend, $all, $cateId);
        } elseif ($dayCount > 1 && $dayCount <= 31) {
            $data = $this->trend($time, 1, $income, $expend, $all, $cateId);
        } elseif ($dayCount > 31 && $dayCount <= 92) {
            $data = $this->trend($time, 3, $income, $expend, $all, $cateId);
        } elseif ($dayCount > 92) {
            $data = $this->trend($time, 30, $income, $expend, $all, $cateId);
        }
     ...
     * @throws BindingResolutionException
     */
    public function trend($time, $num, $incomeLevel, $expendLevel, bool $all = false, array $cateId = [])
    {
        $incomeRank = $expendRank = [];
        $entId      = 1;
        if ($incomeLevel !== '') {
            $incomeRank = $this->dao->getBillRank($time, 1, $entId, $incomeLevel, $this->dao->getSum($time, 1, $incomeLevel, $entId), cateSearch: $cateId);
        }
        if ($expendLevel !== '') {
            $expendRank = $this->dao->getBillRank($time, 0, $entId, $expendLevel, $this->dao->getSum($time, 0, $expendLevel, $entId), cateSearch: $cateId);
        }
        if ($all) {
            if ($num == 0) {
     ...
     * @throws BindingResolutionException
     */
    public function getRankAnalysis(string $time, int $cateId, int $types): array
    {
        $time = explode('-', $time);
        if (count($time) != 2) {
            throw $this->exception('参数错误');
        ...
        return $this->dao->getBillRank($time, $types, $entId, 0, $this->dao->getSum($time, $types, 0, $entId, $cateId), $cateId);
    }
                ...
                [
                    ...
                    'rule'  => [
                        Form::cascader('cate_id', '账目分类')
                            ...
                            ->validate([Form::validateNum()->required()->message('请选择账目分类')]),
                    ],
                ], [
                    'value' => 1,
                    'rule'  => [
