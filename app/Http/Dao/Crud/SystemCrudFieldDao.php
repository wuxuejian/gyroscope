...
class SystemCrudFieldDao extends BaseDao
{
     ...
     * @date 2024/4/12
     */
    public function getFieldTableShowRowSelect(int $crudId, array $select = ['id', 'field_name_en', 'field_name', 'form_value', 'data_dict_id'])
    {
        return $this->getModel()->where('crud_id', $crudId)
            ->where('field_name_en', '<>', 'deleted_at')
            ->where('is_table_show_row', 1)
            ->where('form_value', '<>', CrudFormEnum::FORM_FILE)
            ->select($select)
            ->get()->toArray();
    }
     ...
     * @date 2024/4/19
     */
    public function crudIdByAssociationCrudList(int $crudId, array $master, array $with)
    {
